...
const blogTranslations: Record<string, Partial<Record<Locale, BlogArticleTranslation>>> = {
  ...
  "led-outdoor-lighting-technology": {
    en: {
      category: "Lighting Knowledge",
      title: "LED Outdoor Lighting Technology: From Light Source to Luminaire Reliability",
      description: "A systematic guide to packages, color rendering, thermal design, photometry, glare control, waterproofing, controls and certification.",
      readTime: "15 min read",
    },
    ru: {
      category: "Знания об освещении",
      title: "Технологии наружного LED-освещения: от источника света к надежности светильника",
      description: "Системный разбор корпусировки, цветопередачи, теплоотвода, КСС, антиблика, влагозащиты, управления и сертификации.",
...
export const blogArticles: BlogArticle[] = [
  ...
  },
  {
    ...
    image: "/images/generated/hero-facade-lighting.png",
    date: "2026-07-24",
    readTime: "约 7 分钟阅读",
    intro:
  ...
  {
    slug: "led-outdoor-lighting-technology",
    category: "照明知识",
    title: "LED 户外灯具技术分享：从光源到整灯可靠性",
    description: "基于 43 页 LED 户外产品知识资料，系统梳理封装、显色、散热、配光、防眩、防水、控制系统和认证标准。",
    image: "/images/blog/led-outdoor-product-knowledge/clean/figure-13.jpg",
    date: "2026-08-23",
    readTime: "约 15 分钟阅读",
    intro:
      "LED 户外照明灯具的专业性，不只体现在外观和功率参数上。一个可靠的户外灯具，需要同时处理光源效率、显色表现、散热路径、光学配光、防眩结构、防水工艺、控制系统和认证标准。对于建筑立面、景观、公园、市政道路和商业综合体项目来说，理解这些技术逻辑，可以让选型、设计、采购和验收更有判断依据。",
    sections: [
      {
        heading: "一、先看 LED 光源：封装、芯片和颜色决定基础能力",
        body: [
          "LED 灯具的性能首先来自光源。常见封装形式包括 3528、5050、2835、3030，以及 CREE XPG、XM-L 等更高功率封装。不同封装对应不同的功率承载、发光效率、散热需求和应用场景，不能只看灯珠数量判断灯具好坏。",
          "芯片品牌、封装规格和电流设计会影响亮度、寿命和稳定性。低功率封装适合装饰、轮廓和近距离线性光，高功率封装更适合投光、洗墙和远距离照射。工程选型时，应结合目标照射距离、安装空间和散热条件判断，而不是简单追求更高功率。",
          "彩色 LED 还需要关注波长范围。红光、橙光、黄光、绿光、蓝光和紫光的视觉亮度并不相同，同样封装在不同颜色下的亮度表现也会有差异。白光则需要进一步通过色温来区分暖白、中性白和冷白。",
        ],
      },
      {
        heading: "二、显色指数不能只看 Ra，还要关注 R9",
        body: [
          "在白光 LED 中，显色指数 CRI 是判断光源还原物体颜色能力的重要指标。很多资料会重点标注 Ra，但对于高显色 LED 来说，仅看 Ra 并不充分，还需要关注对饱和红色的特殊显色指数 R9。",
          "户外照明并非所有场景都必须追求高显色，但在商业街区、酒店入口、文化建筑、景观庭院和近人尺度空间中，显色性会直接影响材质、植物、人物肤色和建筑表面的观感。Ra 和 R9 同时较高时，白光表现通常会更自然。",
          "如果项目只是道路或基础功能照明，重点可能是效率、均匀度和眩光控制；如果项目强调商业氛围和空间品质，则应把显色指数纳入选型条件。",
        ],
      },
      {
        heading: "三、亮度与功率不是简单正比",
        body: [
          "很多人会用功率判断亮度，但 LED 灯具的实际光输出受多种因素影响。灯珠光效、驱动效率、玻璃透光率、透镜效率、散热效率和电流设计，都会影响整灯光效。",
          "例如整灯光效可以理解为多个环节共同作用的结果：LED 光效乘以玻璃透光率、透镜透光效率和电源驱动效率。即使光源本身效率较高，如果透镜损耗大、玻璃透光率低、驱动效率不足或散热不良，最终整灯光效仍会明显下降。",
          "增加电流并不等于等比例增加亮度。电流提高后，LED 发热增加，结温上升，光效可能下降，光衰也可能加快。因此专业灯具更重视合理电流设计和热管理，而不是单纯把功率做高。",
        ],
      },
      {
        heading: "四、LM80 和 LM79：判断光衰与整灯性能的重要标准",
        body: [
          "LED 户外灯具长期运行时，不能只看出厂亮度，还要关注光衰。LM80 主要用于评估 LED 光源在一定温度和电流条件下的流明维持能力，是判断光源长期稳定性的参考标准。",
          "LM79 则更多用于整灯层面的光电性能测试，包括光通量、功率、光效、光强分布等。一个成熟的工程灯具，应能提供与项目选型相关的光电数据，而不只是产品图片和功率标注。",
          "在项目采购中，LM80 和 LM79 不一定每个小项目都完整提交，但其背后的逻辑很重要：灯具不仅要初始亮度够，还要在真实户外环境中长期保持稳定。",
        ],
      },
      {
        heading: "五、散热决定 LED 寿命和稳定性",
        body: [
          "LED 的热量需要从芯片、焊点、PCBA、导热介质传递到灯体，再通过灯体与空气进行散热。这个路径中任何一环热阻过大，都会导致结温升高，影响光效、寿命和可靠性。",
          "常见散热概念包括热传导、热容、对流散热和热辐射。金属材料中，银和铜的导热能力较强，但成本、加工和重量限制明显；铝在导热、热容、加工和成本之间取得平衡，因此成为户外 LED 灯体中最常见的散热材料。",
          "对于大功率洗墙灯、投光灯和泛光灯，散热结构不能只看外观。应关注灯体厚度、散热面积、导热路径、PCBA 与灯体接触方式、密封结构是否影响散热，以及安装环境是否有足够空气流动。",
        ],
      },
      {
        heading: "六、配光决定光有没有被用在正确位置",
        body: [
          "户外灯具的专业差异，很大一部分体现在配光。透镜、玻璃、反射结构和出光角度决定了光线如何到达被照面。PMMA、PC 透镜以及普通玻璃、超白玻璃、光学玻璃等材料，会影响透光率、耐候性和最终光效。",
          "配光曲线能够显示不同角度上的光强分布。看配光时，不仅要看标称角度，还要看 50% 光强范围、主光强和溢散光控制。溢散光过多，会造成能量浪费、眩光和不必要的环境光污染。",
          "对于条形灯具和洗墙灯，双角度配光、矩形配光和精准控光非常关键。好的配光不是平均把光分散出去，而是把主要光强投向有效被照面，同时用少量补光改善横向暗区，让墙面亮度更均匀。",
        ],
      },
      {
        heading: "七、防眩光是户外项目高级感的重要分界线",
        body: [
          "户外灯具如果只追求亮度，容易产生眩光。眩光会影响行人舒适度、车辆安全和建筑使用体验，也会让项目显得粗糙。",
          "常见防眩方式包括遮光板、偏光透镜、格栅和特殊配光。遮光板适合控制外溢光，偏光透镜可以改变光线方向，格栅能够限制可视角度，特殊配光则从光学设计阶段减少不必要的溢散光。",
          "在低位上照、高空下照、近人步道、入口、广场和商业街区等场景中，防眩设计尤其重要。好的防眩不是把灯光挡弱，而是在保证有效照明的同时，让人看不到刺眼光源。",
        ],
      },
      {
        heading: "八、防水结构决定户外灯具能否长期运行",
        body: [
          "户外灯具长期面对雨水、灰尘、温差、紫外线和维护环境，防水是可靠性的核心。传统防水方式常见有封胶、防水胶条和结构密封，不同工艺对寿命、外观、维护和一致性都有影响。",
          "封胶方式施工简单，但长期使用中可能受到老化、开裂、热胀冷缩和维修困难的影响。结构防水更强调灯体本身的密封设计，通过合理的机械结构、胶条压缩和出线处理实现防护。",
          "选择户外灯具时，除了看 IP 等级，还要看接线口、防水接头、端盖、玻璃压合、胶条材料、灯体变形控制和维护方式。很多现场故障不是灯体正面进水，而是接线和端部细节处理不到位。",
        ],
      },
      {
        heading: "九、控制系统让灯具从单点照明变成场景资产",
        body: [
          "户外照明控制系统可以分为传统控制、手机终端 APP、互联网集成系统等。对于静态白光项目，控制需求可能只是开关和分时调光；对于景观、商业、文旅和建筑立面项目，则可能涉及分区、调光、调色、动态节目和远程维护。",
          "DMX512 是常见的灯光控制协议，最早由 USITT 发展，用标准数字接口实现控制台与控制设备之间的通信。它具有简单、可靠、灵活的特点，在景观照明、舞台灯光和建筑动态照明中仍被广泛使用。",
          "完整控制系统不只是控制器本身，还包括电源、信号线、地址分配、分区逻辑、无线或有线通信、场景程序和后期维护。项目越复杂，越需要在设计阶段明确控制拓扑和调试流程。",
        ],
      },
      {
        heading: "十、认证标准是进入工程市场的基础门槛",
        body: [
          "户外照明产品不仅要满足效果要求，也要满足产品认证、体系认证和标识标准。不同国家和项目类型可能关注 CE、RoHS、CB、ETL、UL、防护等级、盐雾测试、环境可靠性或质量管理体系等不同资料。",
          "对于出口项目和大型工程项目，认证资料会直接影响投标、报关、验收和客户信任。成熟的照明企业应建立完整的产品资料体系，包括规格书、安装说明、配光文件、检测报告、认证文件和维护指南。",
          "认证并不能替代真实项目测试，但它是判断企业规范度和产品风险的重要基础。采购方应根据项目所在地和应用场景提前确认需要哪些认证，避免后期交付受阻。",
        ],
      },
    ],
    checklist: [
      "确认 LED 封装、芯片品牌、电流设计和应用场景是否匹配。",
      "白光项目关注色温一致性、Ra 和 R9；商业空间不应忽略显色。",
      "不要只看功率，要看整灯光效、配光、安装距离和散热条件。",
      "关注 LM80、LM79 或相关光电测试资料，判断长期稳定性。",
      "大功率户外灯具必须重点评估热传导路径和灯体散热结构。",
      "配光曲线、防眩结构、防水方式和控制系统应与项目场景一起判断。",
      "出口或工程项目提前确认认证、检测报告和安装资料是否完整。",
    ],
    conclusion:
      "LED 户外照明技术是一套系统工程。光源决定基础能力，散热决定寿命，配光决定效果，防眩决定舒适度，防水决定可靠性，控制系统决定场景扩展能力，认证资料决定项目交付边界。真正专业的户外灯具，不是某一个参数突出，而是在光学、结构、电气、控制和工程应用之间取得稳定平衡。",
  },
];
